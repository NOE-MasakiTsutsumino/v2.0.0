...
def draw_sensitivity_histogram(cal, stid, freq, params, mean, m_lower, m_upper, n_lower, n_upper, tolerance_lower, tolerance_upper, dir, day):
    ...
    plt.legend([f"SLM-subMic_diff mean = {str(f'{mean:.2g}')}", f"normal_interval = {n_lower:.2g} to {n_upper:.2g}", f"tolerance = {tolerance_lower + n_lower:.2g} to {tolerance_upper + n_upper:.2g}"], loc='upper left')
...
def plot_histogram_learning_parameters(parameters,stationid,freq,title):
    fig = plt.figure()
