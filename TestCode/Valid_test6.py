...
異常判定時の比較で、95%信頼区間を求めるのを校正直後データのみとし、異常検知対象区間は単に分布の平均値とする。
なぜなら異常検知対象の偏差が大きくなると判定が甘くなってしまうから
"""
...
def do_valid(diffsA, diffsB, p, tolerance):
    ...
    medianB = np.median(diffsB)

    interval = cal_mean_interval(diffsA,p)
    # intervalB = cal_mean_interval(diffsB,p)

    if interval[0] - tolerance > medianB:
        jdg = 1
        return jdg ,interval , medianB
    elif interval[1] + tolerance <  medianB:
        jdg = 2
        return jdg ,interval ,medianB

    return 0 ,interval ,medianB
...
def get_params_calibrated(ID, st_time, freq, num, wav_dir, diffs = None, slm_lvs= None, exclusion = False, SN_check = False):
    ...
    return get_params_calibrated(ID, time, freq, num, wav_dir, diffs, slm_lvs, exclusion = exclusion, SN_check = SN_check)
...
def draw_hist(a,b,a_int,mean_diff):
    """
    ...
    plt.plot(x_a, density_a, color='blue')
    plt.plot(x_b, density_b, color='orangered')

    plt.axvline(np.mean(a), color='blue', linestyle='dashed', linewidth=1)
    plt.axvspan(a_int[0], a_int[1], color="skyblue",alpha = 0.3)

    plt.axvline(np.median(b), color='orangered', linestyle='dashed', linewidth=1)
...
def main():
    ...
    for freq in FREQS:
        diffsA[str(freq)], slm_lv_A[str(freq)]  = get_params_calibrated(ID, caliblation_time, freq, num, WAVE_DIR, exclusion = True, SN_check = SN_check)
    ...
    while 1:
        ...
        for freq in FREQS:
            ...
            diffsB,slm_lv_B  = get_params_period(ID,period[0], period[1], WAVE_DIR, freq, exclusion, SN_check)

            if diffsB != -1:
                fig_dir = os.path.join(SAVE_DIR,period[0].strftime('%Y%m%d'))
                if not os.path.exists(fig_dir): 
                    os.makedirs(fig_dir)
                jdg ,intervalA , _ = do_valid(diffsA[str(freq)], diffsB, p, tolerance)
                if  jdg == 1: # and welch_ttest(slm_lv_A[str(freq)],slm_lv_B,  0.05 ,func = 2) == True:
                    msg = period[0].strftime('%Y%m%d') + " " + str(freq) + " Hz" + " 1"
                    # msg = period[0].strftime('%Y%m%d') + " " + str(freq) + " Hz"  + " 騒音計平均値低い or サブマイク高い"
                    # plt.hist(slm_lv_A[str(freq)],alpha = 0.5, label='SLM_Calibrated',bins = 10)
                    # plt.hist(slm_lv_B,alpha = 0.5, label='SLM_Target',bins = 10)
                    # plt.legend()
                    # plt.savefig(os.path.join(LOG_DIR,period[0].strftime('%Y%m%d') + "_" + str(freq) + "Hz.png"))
                    # plt.close()

                    msg_list.append(msg)
                elif jdg == 2: # and welch_ttest(slm_lv_B, slm_lv_A[str(freq)], 0.05 ,func = 1) == True:
                    # welch_ttest(slm_lv_B, slm_lv_A[str(freq)], 0.05 ,func = 1)
                    msg = period[0].strftime('%Y%m%d') + " " + str(freq) + " Hz" + " 2"
                    # msg = period[0].strftime('%Y%m%d') + " " + str(freq) + " Hz" + " 騒音計平均値高い or サブマイク低い"
                    msg_list.append(msg)
                    # plt.hist(slm_lv_A[str(freq)],alpha = 0.5, label='Calibrated',bins = 10)
                    # plt.hist(slm_lv_B,alpha = 0.5, label='Target',bins = 10)
                    # plt.legend()
                    # plt.savefig(os.path.join(LOG_DIR,period[0].strftime('%Y%m%d') + "_" + str(freq) + "Hz.png"))
                    # plt.close()
        
                fig = draw_hist(diffsA[str(freq)], diffsB, intervalA, tolerance)
