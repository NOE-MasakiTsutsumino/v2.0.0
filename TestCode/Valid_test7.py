...
def main():
    ...
    caliblation_time = datetime.datetime(2022,12,2,23,59,59)
    # 信頼区間推定の水準
    p = 0.95
    # 許される平均値のずれ
    tolerance = 0.3
    ...
    # diffsA = get_params_target(ID, caliblation_time, 1000, num, WAVE_DIR, exclusion = exclusion, SN_check = SN_check, direction ="backward")
    ...
    while 1:
        ...
        for freq in FREQS:
            diffsB = get_params_target(ID, day, freq, num, WAVE_DIR, exclusion = exclusion, SN_check = SN_check, direction = "backward")

            jdg ,intervalA ,intervalB = do_valid(diffsA[str(freq)], diffsB, p, tolerance)
            if  jdg == 1:
                msg = day.strftime('%Y%m%d') + " " + str(freq) + " Hz"  + " 騒音計平均値低い or サブマイク高い"
                msg_list.append(msg)
            elif jdg == 2:
                msg = day.strftime('%Y%m%d') + " " + str(freq) + " Hz" + " 騒音計平均値高い or サブマイク低い"
                msg_list.append(msg)
