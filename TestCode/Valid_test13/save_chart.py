...
def draw_sensitivity_histogram(cal, stid, freq, params, mean, m_lower, m_upper, n_lower, n_upper, tolerance_lower, tolerance_upeer, dir, day):
    """
    結果分析用のヒストグラム画像作成
    """
    ...
    plt.axvspan(n_upper, n_upper + tolerance_upeer, color="lightgreen",alpha = 0.3)

    plt.legend([f"mean = {str(f'{mean:.2g}')}", f"mean_interval = {m_lower:.2g} to {m_upper:.2g}", f"norm_interval = {n_lower:.2g} to {n_upper:.2g}", f"tolerance = {tolerance_lower + n_lower:.2g} to {tolerance_upeer + n_upper:.2g}"], loc='upper left')
