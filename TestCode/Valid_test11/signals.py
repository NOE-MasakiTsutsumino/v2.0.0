...
class SignalProcesses():
    ...
    def cal_CPB_level(self, data, center_freq):

        """
        信号と対象周波数をもらって、オクターブバンドレベルを求める
        """
        practicable_freqs = [125,250,500,1000,2000,4000]

        if center_freq not in practicable_freqs:
            raise Exception(f'分析対象外の周波数。freqは {practicable_freqs}の何れかを選択してください. ')

        fil = self.make_oct_mask(center_freq)

        frame_num = int(np.floor((data.shape[0] - self.fft_n) / self.hop_len)) + 1

        if frame_num < 1:
            raise Exception(f"Error:信号長がFFT点数 %d に満たないのでダメです" % self.fft_n)
        
        rfft = np.fft.rfft

        f = []
        for i in range(frame_num):
            frame = self.window_func * data[(self.hop_len * i):(self.fft_n + (self.hop_len * i))]
    ...
    def cal_CPB_level2(self, signal, oct_freq_mask):
        """
        cal_CPB_levelの簡易バージョン
        """
        ...
        if frame_num < 1:
            #! loggerを起動する処理を入れる
            print(f"Error:信号長がFFT点数 %d に満たないのでダメです" % self.fft_n)
            return False
