...
class Detecter(object):
    ...
    def __init__(self, settings, logger):
        ...
        self.logger = logger
        self.sp = SignalProcesses(settings)
        ...
        self.logger.app.debug("Detecter")
...
class Learning(Detecter):
    ...
    def __init__(self, settings, logger):
        ...
        self.logger.app.debug("Learning")
    ...
    def _get_normal_params_freq(self, stid, st_time, freq, proportion) -> tuple:
        ...
        # パラメータが指定数になるまでループ処理
        while len(diffs) < self.normaly_sample_size:

            event_time, path = self.get_target_event_time(stid, time)

            if event_time == -1:
                #! Critical
                msg = "失敗:校正完了後のデータ不足。正常データが得られませんでした。[必要数:{},不足数{}]".format(self.normaly_sample_size,self.normaly_sample_size - len(diffs))
                self.logger.app.critical(msg)
                raise Exception(msg)
            ...
            slm_lv = self.sp.cal_CPB_level(slm_data, freq)
            sub_lv = self.sp.cal_CPB_level(sub_data, freq)
        ...
        bottom, up = self.cal_mean_interval(diffs, proportion)
    ...
    def _cal_normal_params_function(self, stid, calibrated_time, freqs, propation) -> dict:
        ...
        ret = {}
        ret["norm_var"] = {}
        ret["norm_interval_upper"] = {}
        ret["norm_interval_lower"] = {}

        for freq in freqs:
            var, bottom, up = self._get_normal_params_freq(stid, calibrated_time, freq, propation)

            ret["norm_var"][str(freq)] = var
            ret["norm_interval_lower"][str(freq)] = bottom
            ret["norm_interval_upper"][str(freq)] = up
    ...
    def fit(self):
        ...
        for stid in self.target_station_list:
            ...
            self.norm[stid] = self._cal_normal_params_function(stid, ct, freqs ,p)
            self.logger.app.debug("learning-{}-complete".format(stid))

        with open('NORMAL_PARAMETERS.json', 'w') as f:
...
class Station(Detecter):

    def __init__(self, stid, name, execution, calibrated_time,
                target_frequency = None, proportion = None, anormaly_tolerance = None, sensitivity_tolerance = None):

            self.stid = stid
            self.name = name
            self.execution = execution
            self.calibrated_time = calibrated_time