...
def main():
    ...
    # data = get_params_target(ID, caliblation_time, 250, num, WAVE_DIR, exclusion = exclusion, SN_check = SN_check)
    # delete_outlier_var(data,250)
    ...
    for freq in FREQS:
        ...
        NORMAL_VAR_LIST[str(freq)] = np.var(diffsA[str(freq)], ddof = 1) + 0.00
    ...
    while 1:
        ...
        for freq in FREQS:
            diffsB = get_params_target(ID, day, freq, num, WAVE_DIR, exclusion = exclusion, SN_check = SN_check, var_check = True, direction = "backward")

            jdg ,intervalA ,intervalB = do_valid(diffsA[str(freq)], diffsB, p, tolerance)
            if  jdg == 1:
                msg = day.strftime('%Y%m%d') + " " + str(freq) + " Hz"  + " 1"
                msg_list.append(msg)
            elif jdg == 2:
                msg = day.strftime('%Y%m%d') + " " + str(freq) + " Hz" + " 2"
                msg_list.append(msg)
